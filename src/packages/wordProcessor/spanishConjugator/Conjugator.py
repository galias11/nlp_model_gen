...
class Conjugator:
    ...
    def set_general_config(self, general_config):
        self.configs['config'] = general_config

    def set_irregular_verb_groups_config(self, irregular_verb_groups_config):
        self.configs['irregular_verb_groups'] = irregular_verb_groups_config

    def set_irregular_verb_exceptions_config(self, irregular_verb_exceptions_config):
        self.configs['irregular_verb_exceptions_config'] = irregular_verb_exceptions_config
    ...
    def generar_diccionario_conjugacion(self, verb):
        if not any(fnmatch.fnmatch(verb, suffix) for suffix in self.configs['config']['verb_suffixes']):
            return {}
        key = self.extract_key(verb)
        if key is not None:
            return self.generar_dict_irregular_concatenado(key, verb)
        else:
    ...
    def extract_key(self, verb):
        for key in self.configs['irregular_verb_exceptions']:
            if fnmatch.fnmatch(verb, '*' + key) and key not in self.configs['config']['irregular_suffix_exceptions']:
                if key != verb:
                    return key
                else:
                    return ''
    ...
    # verbo no válido.
    def empty_dict(self):
        return self.configs['config']['empty_conj_dict']

    # Construye la primera fila de la tabla de conjugación
    def contruir_primera_fila(self, conjugation):
        return [
            self.configs['config']['table_headers'][0],
            [self.configs['config']['row_headers'][0][0], conjugation['pres'][0], conjugation['pret_imperf'][0], conjugation['pret_perf'][0], conjugation['condA'][0]],
            [self.configs['config']['row_headers'][0][1], conjugation['pres'][1], conjugation['pret_imperf'][1], conjugation['pret_perf'][1], conjugation['condA'][1]],
            [self.configs['config']['row_headers'][0][2], conjugation['pres'][2], conjugation['pret_imperf'][2], conjugation['pret_perf'][2], conjugation['condA'][2]],
            [self.configs['config']['row_headers'][0][3], conjugation['pres'][3], conjugation['pret_imperf'][3], conjugation['pret_perf'][3], conjugation['condA'][3]],
            [self.configs['config']['row_headers'][0][4], conjugation['pres'][4], conjugation['pret_imperf'][4], conjugation['pret_perf'][4], conjugation['condA'][4]],
            [self.configs['config']['row_headers'][0][5], conjugation['pres'][5], conjugation['pret_imperf'][5], conjugation['pret_perf'][5], conjugation['condA'][5]]
        ]

    # Construye la segunda fila de la tabla de conjugación
    def construir_segunda_fila(self, conjugation):
        return [
            self.configs['config']['table_headers'][1],
            [self.configs['config']['row_headers'][0][0], conjugation['fut'][0], conjugation['condB'][0], conjugation['impA'][0], conjugation['impB'][0], conjugation['impC'][0]],
            [self.configs['config']['row_headers'][0][1], conjugation['fut'][1], conjugation['condB'][1], conjugation['impA'][1], conjugation['impB'][1], conjugation['impC'][1]],
            [self.configs['config']['row_headers'][0][2], conjugation['fut'][2], conjugation['condB'][2], conjugation['impA'][2], conjugation['impB'][2], conjugation['impC'][2]],
            [self.configs['config']['row_headers'][0][3], conjugation['fut'][3], conjugation['condB'][3], conjugation['impA'][3], conjugation['impB'][3], conjugation['impC'][3]],
            [self.configs['config']['row_headers'][0][4], conjugation['fut'][4], conjugation['condB'][4], conjugation['impA'][4], conjugation['impB'][4], conjugation['impC'][4]],
            [self.configs['config']['row_headers'][0][5], conjugation['fut'][5], conjugation['condB'][5], conjugation['impA'][5], conjugation['impB'][5], conjugation['impC'][5]]
        ]

    # Construye la tercera fila de la tabla de conjugación
    def construir_tercera_fila(self, conjugation):
        return [
            self.configs['config']['table_headers'][2],
            [self.configs['config']['row_headers'][1][0], conjugation['inf'][0]],
            [self.configs['config']['row_headers'][1][1], conjugation['ger'][0]],
            [self.configs['config']['row_headers'][1][2], conjugation['part'][0]],
            [self.configs['config']['row_headers'][1][3], conjugation['part'][1]],
            [self.configs['config']['row_headers'][1][4], conjugation['part'][2]]
        ]
    ...
    def table_view(self, verb):
        ...
        table_data_1 = self.contruir_primera_fila(conjugation)
        table_01 = AsciiTable(table_data_1)
        table_data_2 = self.construir_segunda_fila(conjugation)
        table_02 = AsciiTable(table_data_2)
        table_data_3 = self.construir_tercera_fila(conjugation)
